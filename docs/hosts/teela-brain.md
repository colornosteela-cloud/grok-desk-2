...
`~/.grok/config.toml` default is `qwen38-27b-q5` with `base_url=http://127.0.0.1:8081/v1` (llama.cpp, rewritten through this deskd `/v1/llm` when the picker uses a catalog id). Brain bots list this file's models: **Qwen 3.8 27B Q5** (local GPU) plus Grok 4.6 / 4.5 (cloud via `grok login`). Occupied GPU rows are grayed; Grok cloud stays selectable. Body's catalog is not shown here.
...
llama.cpp loopback servers and Grok Desk, user systemd (linger on so they start at boot without a login):

| Engine | Port | Service | Role |
| --- | --- | --- | --- |
| Qwen3.8-27B Q5 + MTP + vision | `8081` | `teela-qwen38-27b.service` (**enabled**) | Teela brain, picker `qwen38-27b-q5` |
| Grok Desk MiniOS | `8742` / `8743` | `grok-desk.service` (**enabled**, after 27B) | UI, Teela executive, cluster |
...
Grok Desk autostart: `contrib/grok-desk.service` installed as `~/.config/systemd/user/grok-desk.service`, `systemctl --user enable --now grok-desk.service`, and `loginctl enable-linger` so it (and the 27B unit) come up on reboot. `./start.sh` is a no-op while that unit holds `:8742`.

Short voice-chat replies go through the 27B lane while it holds the GPUs. deskd refuses a second daemon (pidfile + flock in `$XDG_RUNTIME_DIR/grok-desk/deskd.pid`).

**MiniOS tool loop (not Grok Build):** Teela body/desktop turns call llama.cpp with a small OpenAI tool list (`teela_*`, `robot_*`, `desktop_*`, `memory_write` / `memory_retrieve`) and run those tools in deskd. Session facts from `workspace/.memory/` are injected into the system prompt (Q5 budget 16k memory tokens on a 262k window). No `search_tool` / `use_tool`. Grok Build ACP is only the fallback if the local engine is down. Body Bot stays on Grok Build.
...
- **UI files are served `no-store`** (and `app.js` / `grokbot-ui.js` carry a `?v=` bust). Hard-refresh after a pull so a stale tab cannot speak DMs.
- `GET /v1/voice/health` → `{tts, stt, voice}`.
- **Phones/other PCs**: mic capture needs a secure context, so deskd also serves **HTTPS on `listen_port+1`** (8743) with a self-signed cert in `~/.grok/certs/` (accept it once per device). TTS playback works over plain HTTP.
